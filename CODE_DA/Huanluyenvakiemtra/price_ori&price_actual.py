...
X = np.array(data['price_ori']).reshape(-1, 1)  # price_ori là biến độc lập
Y = np.array(data['price_actual']).reshape(-1, 1)  # price_actual là biến phụ thuộc
...
print("Kích thước của tập kiểm tra (X_test, y_test): ", X_test.shape, y_test.shape)
...
plt.title("Relationship Between Original Price and Actual Price", fontsize=14)
plt.xlabel("Original Price (price_ori)", fontsize=12)
plt.ylabel("Actual Price (price_actual)", fontsize=12)
plt.legend()
plt.grid(True)
